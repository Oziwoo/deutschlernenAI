...
const STATUS_LABELS = {
  [STATUS.NEW]:      { label: 'Новое',      cls: 'bg-stone-100 text-stone-500' },
  [STATUS.LEARNING]: { label: 'Учится',     cls: 'bg-amber-100 text-amber-600' },
  [STATUS.REVIEW]:   { label: 'Повторение', cls: 'bg-blue-100 text-blue-600'   },
  [STATUS.MASTERED]: { label: 'Усвоено',    cls: 'bg-green-100 text-green-600' },
}
...
function WordModal({ word, progress, onClose }) {
  ...
  const status   = progress?.status || STATUS.NEW

  const load = useCallback(async () => {
    if (expl || loading) return
    setLoading(true)
    try {
      const r = await fetch(`/api/explain?id=${word.id}&word=${encodeURIComponent(word.word)}&article=${word.article||''}&category=${word.category}`)
      const d = await r.json()
      setExpl(d.explanation || '—')
    } catch {
      setExpl('Не удалось загрузить объяснение.')
    } finally {
      setLoading(false)
    }
  }, [word.id, expl, loading])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 animate-fade-in" onClick={onClose}>
      <div
        className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-stone-200 animate-slide-up overflow-hidden max-h-[85vh] flex flex-col"
        ...
        <div className="flex items-start justify-between p-5 border-b border-stone-100">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_LABELS[status].cls}`}>
...
export default function BrowseMode({ progressMap }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      list = list.filter(w => {
        const s = progressMap[w.id]?.status || STATUS.NEW
    ...
    <div className="py-6 animate-fade-in">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-hide">
        ...
        {Object.entries(CATEGORIES).map(([id, name]) => (
          <button
            key={id}
            onClick={() => setCat(catFilter === id ? 'all' : id)}
      ...
      <div className="flex gap-2 mb-4">
        {[
          ...
          { value: STATUS.NEW,      label: 'Новые' },
          { value: STATUS.LEARNING, label: 'Учится' },
          { value: STATUS.REVIEW,   label: 'Повторение' },
          { value: STATUS.MASTERED, label: 'Усвоено' },
        ].map(s => (
          <button
            key={s.value}
